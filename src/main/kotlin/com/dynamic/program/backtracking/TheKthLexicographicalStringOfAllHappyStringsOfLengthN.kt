...
class TheKthLexicographicalStringOfAllHappyStringsOfLengthN {
    fun getHappyString(n: Int, k: Int): String {
        ...
        println(result.size)
        println(result)
        if (k - 1 > result.size - 1) return ""
        return result[k-1]
    }

    fun backtrack(cur: String, level: Int, result: MutableList<String>) {
        if (level == 0) {
            result.add(cur)
            return
        }
        ...
        for(i in 0 until 3) {
            val ch = 'a'+i
            if (cur.isEmpty() || cur[cur.length - 1] != ch) {
                backtrack("$cur$ch", level-1, result)
            }
        }
    }
}
...
fun main() {
    println(TheKthLexicographicalStringOfAllHappyStringsOfLengthN().getHappyString(5,9))
}