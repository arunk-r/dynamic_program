...
class MaximumSplitOfPositiveEvenIntegers {
    fun maximumEvenSplit(finalSum: Long): List<Long> {
        ...
        val diff = sum - finalSum
        if (diff > 0) {
            val last = lst.removeAt(lst.size - 1)
            sum -= last

            val trueLast = lst.removeAt(lst.size - 1)
            lst.add(trueLast + (finalSum - sum))
        }
        return lst
    }
}
...
fun main() {
    println(MaximumSplitOfPositiveEvenIntegers().maximumEvenSplit(18))
}