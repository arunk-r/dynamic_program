...
class SuccessfulPairsOfSpellsAndPotions {
    fun successfulPairs(spells: IntArray, potions: IntArray, success: Long): IntArray {
        ...
        for (i in spells.indices) {
            val j: Int = binarySearch(potions, success / spells[i].toDouble())
            ans[i] = m - j
        }

        return ans
    }
    private fun binarySearch(arr: IntArray, target: Double): Int {
        var left = 0
...
fun main() {
    println(
        SuccessfulPairsOfSpellsAndPotions().successfulPairs(intArrayOf(5, 1, 3), intArrayOf(1, 2, 3, 4, 5), 7).toList()
    )
}