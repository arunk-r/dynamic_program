...
class ArrayQuadruplet {
    fun quadruplet(nums: IntArray, target: Int): List<Int> {
        nums.sort()
        for (i in nums.indices) {
            for (j in i+1 until nums.size) {
                ...
                while (low < high) {
                    val nt = nums[i] + nums[j] + nums[low] + nums[high]
                    if (nt == target) {
                        return mutableListOf(nums[i], nums[j], nums[low], nums[high])
                    } else if (nt > target) {
                        high--
                    } else {
                        low++
...
fun main() {
    println(ArrayQuadruplet().quadruplet(intArrayOf(2, 7, 4, 0, 9, 5, 1, 3), 20))
}
