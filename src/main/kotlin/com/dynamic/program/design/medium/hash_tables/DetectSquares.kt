...
class DetectSquares {
    ...
    private val pairCnt = HashMap<Data, Int>()
    private val lst = mutableListOf<Data>()

    fun add(point: IntArray) {
        val (x, y) = point
        val p = Data(x, y)
        pairCnt[p] = pairCnt.getOrDefault(p, 0) + 1
        lst.add(p)
    }
    ...
    fun count(point: IntArray): Int {
        ...
        for ((px, py) in lst) {
            ...
            sum += pairCnt.getOrDefault(Data(x, py), 0) * pairCnt.getOrDefault(Data(px, y), 0)
        }
        return sum
    }
}
