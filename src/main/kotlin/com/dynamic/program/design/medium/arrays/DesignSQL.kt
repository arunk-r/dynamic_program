 ...
 */
class DesignSQL(names: List<String>, columns: List<Int>) {

    class Data(val colSize: Int) {
        ...
        fun select(id: Int, col: Int): String {
            return map[id]?.get(col-1) ?: ""
        }
    ...
    private val namesDic = hashMapOf<String, Data>()

    init {
        for(i in names.indices) {
            namesDic[names[i]] = Data(columns[i])
        }
    }

    fun insertRow(name: String, row: List<String>) {
        namesDic[name]!!.insert(row)
    }

    fun deleteRow(name: String, rowId: Int) {
        namesDic[name]!!.delete(rowId)
    }

    fun selectCell(name: String, rowId: Int, columnId: Int): String {
        return namesDic[name]!!.select(rowId, columnId)
    }
