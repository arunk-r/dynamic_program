package com.dynamic.program.bit.hard


/**
 * 864. Shortest Path to Get All Keys
Hard

Airbnb
Uber
Palantir Technologies
You are given an m x n grid grid where:

'.' is an empty cell.
'#' is a wall.
'@' is the starting point.
Lowercase letters represent keys.
Uppercase letters represent locks.
You start at the starting point and one move consists of walking one space in one of the four cardinal directions. You cannot walk outside the grid, or walk into a wall.

If you walk over a key, you can pick it up and you cannot walk over a lock unless you have its corresponding key.

For some 1 <= k <= 6, there is exactly one lowercase and one uppercase letter of the first k letters of the English alphabet in the grid. This means that there is exactly one key for each lock, and one lock for each key; and also that the letters used to represent the keys and locks were chosen in the same order as the English alphabet.

Return the lowest number of moves to acquire all keys. If it is impossible, return -1.



Example 1:


Input: grid = ["@.a..","###.#","b.A.B"]
Output: 8
Explanation: Note that the goal is to obtain all the keys not to open all the locks.
Example 2:


Input: grid = ["@..aA","..B#.","....b"]
Output: 6
Example 3:


Input: grid = ["@Aa"]
Output: -1


Constraints:

m == grid.length
n == grid[i].length
1 <= m, n <= 30
grid[i][j] is either an English letter, '.', '#', or '@'.
The number of keys in the grid is in the range [1, 6].
Each key in the grid is unique.
Each key in the grid has a matching lock.
 */
class ShortestPathToGetAllKeys {
    data class State(var keys: Int, var i: Int, var j: Int)

    fun shortestPathAllKeys(grid: Array<String>): Int {
        var x = -1
        var y = -1
        val m = grid.size
        val n = grid[0].length
        var max = -1
        for (i in 0 until m) {
            for (j in 0 until n) {
                val c = grid[i][j]
                if (c == '@') {
                    x = i
                    y = j
                }
                if (c in 'a'..'f') {
                    max = Math.max(c.toInt() - 'a'.code + 1, max)
                }
            }
        }
        val start = State(0, x, y)
        val q = ArrayDeque<State>()
        val visited: MutableSet<String> = HashSet()
        visited.add("0 $x $y")
        q.addLast(start)
        val dirs = arrayOf(intArrayOf(0, 1), intArrayOf(1, 0), intArrayOf(0, -1), intArrayOf(-1, 0))
        var step = 0
        while (!q.isEmpty()) {
            var size: Int = q.size
            while (size-- > 0) {
                val cur: State = q.removeFirst()
                if (cur.keys == (1 shl max) - 1) {
                    return step
                }
                for (dir in dirs) {
                    val i = cur.i + dir[0]
                    val j = cur.j + dir[1]
                    var keys = cur.keys
                    if (i in 0 until m && j >= 0 && j < n) {
                        val c = grid[i][j]
                        if (c == '#') {
                            continue
                        }
                        if (c in 'a'..'f') {
                            keys = keys or (1 shl c.code - 'a'.code)
                        }
                        if (c in 'A'..'F' && keys shr c.code - 'A'.code and 1 == 0) {
                            continue
                        }
                        if (!visited.contains("$keys $i $j")) {
                            visited.add("$keys $i $j")
                            q.addLast(State(keys, i, j))
                        }
                    }
                }
            }
            step++
        }
        return -1
    }
}
