package com.dynamic.program.easy.tournament_winner
...
fun tournamentWinner(competitions: List<List<String>>, results: List<Int>): String {
    ...
    for (t in competitions.indices) {
        val teams = competitions[t]
        var ownTeam:String = ""
        ownTeam = if (results[t] == 0) {
            teams[1]
        } else {
            teams[0]
        }
        if(map.contains(ownTeam)) {
            map[ownTeam] = 3 + map[ownTeam]!!
        } else {
            map[ownTeam] = 3
        }
    }
...
fun main() {
    println(tournamentWinner(listOf(
        listOf("HTML", "C#"),
        listOf("C#", "Python"),
        listOf("Python", "HTML")
    ), listOf(0, 0, 1)))
